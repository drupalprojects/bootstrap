...
function bootstrap_preprocess_region(&$variables) {
  ...
  switch ($region) {
    case 'navigation':
      ...
      if ($config->get('navbar_position') !== '') {
        $attributes['class'][] = 'navbar-' . $config->get('navbar_position');
      }
      else {
        $attributes['class'][] = 'container';
      }
      if ($config->get('navbar_inverse')) {
  ...
  if (!isset($wells)) {
    foreach ($regions as $name => $title) {
      $wells[$name] = $config->get('region_well-' . $name) ? : FALSE;
    }
  }
  if ($wells[$region]) {
  ...
  if (!isset($region_columns)) {
    foreach ($regions as $name => $title) {
      $region_columns[$name] = $config->get('region_grid-' . $name) ? : 0;
    }
    $columns = $config->get('grid_columns') ? : 12;
    foreach ($regions as $name => $title) {
      if ($dynamic_regions = $config->get('region_grid_dynamic-' . $name) ? : array()) {
  ...
  if ($region_columns[$region]) {
    $attributes['class'][] = ($config->get('grid_class_prefix') ? : 'col-sm') . '-' . $region_columns[$region];
  }
}
