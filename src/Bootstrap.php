...
class Bootstrap {
  ...
  public static function alter($function, &$data, &$context1 = NULL, &$context2 = NULL, $namespace = NULL) {
    ...
    $cache = $theme->cache();
    $alters = $cache->get('alters', []);

    // Determine if the function has a valid class counterpart.
    if (!isset($alters[$function])) {
      $alters[$function] = FALSE;
      ...
      foreach ($classes as $name) {
        ...
        if ($class && $class->hasMethod('alter') && (
          ...
          $alters[$function] = $name;
          break;
        }
      }
      $cache->set('alters', $alters);
    }

    // Only continue if class is valid.
    if ($alters[$function] && ($reflection = static::reflectionClass($alters[$function]))) {
